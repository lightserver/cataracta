...
class DomainActor[O, EVENT](
  ...
  private def loadEvents(self: BadActorRef[EventWrapper ]) = {
    val ctx = new NullContext(nodeRef, Address(Local, path = path))
    loading = true
    val maxIdFuture = storage.loadEvents(domain, ctx, self)

    maxIdFuture.onSuccess {
      case maxId => {
        self.send(EndLoading)
  ...
  override def receive(e: EventWrapper, self: BadActorRef[EventWrapper ]): Unit = {
    e match {
      case LoadDomainCommand => loadEvents(self)
      case ev: SendEventCommand => {

        val result = domain.receiveEvent(ev.event, ev.ctx)
        if (result.persist && !loading) {
